...
const LOC_ICONS = {
  [LOC_TYPES.BATTLE]: icon('crossed-swords', 20, '#c8a96e'),
  [LOC_TYPES.ELITE]: icon('skull', 20, '#e05555'),
  [LOC_TYPES.MINI_BOSS]: icon('dragon-small', 20, '#e05555'),
  [LOC_TYPES.BOSS]: icon('dragon', 20, '#ff4444'),
  [LOC_TYPES.REST]: icon('campfire', 20, '#f0a030'),
  [LOC_TYPES.SHOP]: icon('shop', 20, '#c8a96e'),
  [LOC_TYPES.EVENT]: icon('question', 20, '#8888cc'),
  [LOC_TYPES.NPC]: icon('speech', 20, '#88cc88'),
  [LOC_TYPES.TREASURE]: icon('chest', 20, '#c8a96e'),
};
...
const WORLD = {
  biomes: {
    ...
    mountain: {
      name: 'Stormbreak Mountains',
      description: 'Jagged peaks scrape a storm-torn sky. Lightning strikes from cloudless air. The icy summit holds the shattered remains of the old sky-temple.',
      cardPool: ['gust', 'lightning', 'wind_shield', 'tailwind', 'second_wind', 'cleanse', 'dragon_claw', 'dragon_scales', 'flame_shield'],
      enemyPool: ['storm_caller', 'iron_golem'],
      elitePool: ['iron_golem'],
      palette: { bg: '#0a1020', path: 'rgba(120, 180, 220, 0.35)', accent: '#7ab8e0' },
    },
  },

  locations: {
    // === WHISPERING COAST ===
    // Positions calibrated to world_map_bg.png (3600x480 canvas, TILE_SIZE=70). Coast ~0-14, Forest ~14-30, Mountains ~30-50.
    ...
    misthaven_village: {
      ...
      npc: {
        ...
        rewardDialogue: [
          'You have returned! And I can feel it — the storm drake\'s grip on the gate has broken.',
          'I knew you had the strength, traveler. The Thornwood Forest lies open before you now.',
          'But the forest is not like the coast. The corruption runs deeper there. The trees themselves have turned.',
          'Before you go, take this. It is a fragment of the old guardian spirit that once protected these lands.',
          'It will shield you and mend your wounds when the darkness closes in. May it serve you well in the Thornwood.',
        ],
        npcReward: 'elder_spirit',
        npcRewardCondition: 'thornwood_gate',
      },
    },
    whispering_cliffs: {
    ...
    veiled_sea: {
      ...
      x: 7.2, y: 2.3,
      paths: ['shore_market', 'windward_lighthouse', 'foggy_ambush'],
    ...
    foggy_ambush: {
      ...
      type: LOC_TYPES.BATTLE,
      x: 9.5, y: 2.2,
      paths: ['veiled_sea', 'reef_shallows'],
      description: 'The fog thickens between the sea and the bay. Something moves in the mist — too late to turn back.',
      image: 'images/loc_misty_shore.png',
      enemy: 'young_drake',
      goldReward: 10,
    },
    ...
    thornwood_gate: {
      ...
      miniBossVictory: {
        ...
        lines: [
          ...
          'Ancient power flows through you as you clasp it. You feel your body strengthen.',
          'Beyond the fallen beast, the twisted trees of the Thornwood Forest beckon. The path forward is open.',
        ],
        reward: 'Tidecaller\'s Charm',
        rewardDesc: '+10 Max HP',
        maxHpBonus: 10,
      },
    },
    ...
    spring_of_tides: {
      ...
      type: LOC_TYPES.REST,
      x: 16.5, y: 3.5,
      paths: ['thornwood_gate', 'forest_edge', 'hermits_hut'],
    ...
    forest_edge: {
      ...
      type: LOC_TYPES.BATTLE,
      x: 17.9, y: 5.6,
      paths: ['spring_of_tides', 'hermits_hut'],
    ...
    hermits_hut: {
      ...
      type: LOC_TYPES.NPC,
      x: 19.1, y: 3.7,
      paths: ['forest_edge', 'woodcutters_camp', 'mushroom_grove', 'spring_of_tides'],
    ...
    mushroom_grove: {
      ...
      type: LOC_TYPES.EVENT,
      x: 20.1, y: 2.3,
      paths: ['hermits_hut', 'woodcutters_camp'],
    ...
    ruined_bridge: {
      ...
      type: LOC_TYPES.BATTLE,
      x: 21.5, y: 4.4,
      paths: ['ancient_stump', 'woodcutters_camp'],
    ...
    deep_woods: {
      ...
      type: LOC_TYPES.BATTLE,
      x: 28.4, y: 2.9,
      paths: ['forest_heart', 'elder_tree', 'warded_pass'],
    ...
    woodcutters_camp: {
      ...
      type: LOC_TYPES.SHOP,
      x: 21.1, y: 3.1,
      paths: ['hermits_hut', 'mushroom_grove', 'ruined_bridge'],
    ...
    corrupted_shrine: {
      ...
      type: LOC_TYPES.EVENT,
      x: 22.7, y: 6.3,
      paths: ['forest_heart', 'ancient_stump'],
    ...
    elder_tree: {
      ...
      type: LOC_TYPES.EVENT,
      x: 26.1, y: 2.4,
      paths: ['ancient_stump', 'deep_woods'],
    ...
    // The Warded Pass at the forest/mountain boundary is sealed by an ancient
    // storm-binding spell. Defeating Forest Heart grants the storm_shield
    // blessing, which dispels the ward and opens the way.
    warded_pass: {
      name: 'Warded Pass',
      biome: 'mountain',
      type: LOC_TYPES.BATTLE,
      x: 33.1, y: 4,
      paths: ['forest_heart', 'mountain_shrine', 'deep_woods'],
      requiresBlessing: 'storm_shield',
      description: 'Glowing runes blaze across an archway of stone — an old storm-binding ward placed by the Forest Guardian. Only those who carry its blessing may pass without being seared by lightning. Beyond, a lone Storm Caller watches the trail.',
      image: 'images/loc_dark_forest.png',
      enemy: 'storm_caller',
      goldReward: 18,
    },
    rope_bridge: {
      ...
      type: LOC_TYPES.EVENT,
      x: 39.2, y: 3.8,
      paths: ['abandoned_mine', 'echo_cave', 'mountain_shrine'],
      description: 'A rotted rope bridge sways over a bottomless ravine. Mist boils up from below. Each plank looks ready to give.',
      image: 'images/loc_dark_forest.png',
      eventKey: 'rope_bridge_event',
    },
    mountain_shrine: {
      ...
      type: LOC_TYPES.NPC,
      x: 37.1, y: 4.7,
      paths: ['warded_pass', 'storm_peak', 'rope_bridge', 'summit_camp'],
      description: 'A weathered stone shrine clings to a ledge. Prayer flags whip in the thin air. A robed figure sits in meditation.',
      image: 'images/loc_village.png',
      npc: {
        name: 'Mountain Seer',
        image: 'images/npc_elder.png',
        icon: icon('book', 24, '#c8a96e'),
        dialogue: [
          'You carry the blessing of the fallen treant. The ward opened for you — good.',
          'I have watched these mountains for forty winters. The storms were not always like this.',
          'When the third seal broke, lightning began to strike from a cloudless sky. The snow turned hungry. The Skyfall Temple at the peak was swallowed by the glacier in a single night.',
          'A sky-archon was set to guard that temple long ago. The corruption has twisted it. It now holds what you need to reach the volcanic heart — the Flame Ward.',
          'Climb carefully. Rest at the eagle\'s nest before you face it.',
        ],
      },
    },
    abandoned_mine: {
      ...
      type: LOC_TYPES.BATTLE,
      x: 41.2, y: 3.1,
      paths: ['rope_bridge', 'echo_cave', 'eagles_nest'],
      description: 'Rusted rails lead into the dark mouth of an abandoned iron mine. A hulking shape stirs in the shadows — something the miners left behind.',
      image: 'images/loc_crystal_cave.png',
      enemy: 'iron_golem',
      goldReward: 20,
    },
    echo_cave: {
      ...
      type: LOC_TYPES.EVENT,
      x: 42.8, y: 2.5,
      paths: ['rope_bridge', 'abandoned_mine', 'eagles_nest'],
      description: 'A shallow cave where voices echo strangely — your own whispers come back changed, as if answered.',
      image: 'images/loc_crystal_cave.png',
      eventKey: 'echo_cave_event',
    },
    eagles_nest: {
      name: "Eagle's Nest",
      biome: 'mountain',
      type: LOC_TYPES.REST,
      x: 42.1, y: 1.1,
      paths: ['echo_cave', 'storm_peak', 'abandoned_mine', 'skyfall_temple'],
      description: 'A wind-carved ledge high on the cliff, marked by the bleached skeleton of a great eagle. A small fire pit offers shelter from the wind.',
      image: 'images/loc_wanderers_camp.png',
    },
    storm_peak: {
      name: 'Storm Peak',
      ...
      type: LOC_TYPES.ELITE,
      x: 37.1, y: 1.3,
      paths: ['eagles_nest', 'mountain_shrine'],
      description: 'Lightning strikes this bare summit over and over from a clear sky. A towering Iron Golem, its surface seared blue with lightning, waits at the peak.',
      image: 'images/loc_dragons_lair.png',
      enemy: 'iron_golem',
      goldReward: 28,
      specialReward: 'ancient_ward',
    ...
    summit_camp: {
      ...
      type: LOC_TYPES.SHOP,
      x: 43.3, y: 5.3,
      paths: ['crystal_cavern', 'mountain_shrine'],
      description: 'A mountaineer\'s trading post clings to the col. Tents, pack animals, and the smell of salted meat. They sell what they can carry.',
      image: 'images/loc_village.png',
    },
    crystal_cavern: {
      name: 'Crystal Cavern',
      biome: 'mountain',
      type: LOC_TYPES.TREASURE,
      x: 47.4, y: 5.2,
      paths: ['summit_camp', 'skyfall_temple'],
      description: 'Pale blue crystals jut from the cavern walls, humming with stored lightning. A single chest sits at the center, half-buried in ice.',
      image: 'images/loc_crystal_cave.png',
    },
    skyfall_temple: {
      name: 'Skyfall Temple',
      ...
      type: LOC_TYPES.MINI_BOSS,
      x: 47.1, y: 2.2,
      paths: ['crystal_cavern', 'eagles_nest'],
      description: 'The ruined sky-temple emerges from the glacier at the peak — cracked columns and a broken dome, half-swallowed by ice. Electric-blue runes still pulse in the stone. A Sky-Archon waits in the nave, wreathed in stormlight.',
      image: 'images/loc_volcano_peak.png',
      enemy: 'sky_archon',
      goldReward: 40,
      blessing: 'flame_ward',
...
const EVENTS = {
  sea_cave_event: {
    ...
    choices: [
      { text: 'Open the chest (risk)', cost: { hp: 10 }, reward: { chestReward: true, cardCount: 3 }, result: 'The chest\'s guardian spirit lashes out — but you claim the power within.' },
  ...
  rope_bridge_event: {
    ...
    description: 'The bridge creaks in the wind. A few of the planks are missing entirely. Far below, mist swallows everything.',
    choices: [
      { text: 'Cross at a run', cost: { hp: 8 }, reward: { cardReward: true, cardCount: 3 }, result: 'You sprint across as rotted planks give way behind you. A few loose splinters bite you, but you make it — and find a cache left by a previous traveler who wasn\'t so lucky.' },
      { text: 'Cross one plank at a time', cost: null, reward: { heal: 6 }, result: 'You pick your way across, breath held. The wind almost takes you twice, but you reach the far side intact and the adrenaline clears your head.' },
      { text: 'Cut the bridge free after crossing', cost: null, reward: { gold: 12 }, result: 'You cross carefully, then cut the ropes. A brass clasp tumbles off with the bridge — you catch it.' },
    ],
  },
  echo_cave_event: {
    title: 'The Echo Cave',
    description: 'Your footsteps come back wrong. Whispers that sound like your own voice — but the words are older, and they know your name.',
    choices: [
      { text: 'Speak a true name', cost: null, reward: { removeCard: true }, result: 'You name something heavy, and the cave takes it from you. Something in your deck is gone — and you feel lighter.' },
      { text: 'Listen to the echoes', cost: null, reward: { cardReward: true, cardCount: 2 }, result: 'The voices teach you an old technique. You learn something you did not know you had forgotten.' },
      { text: 'Shout a challenge', cost: { hp: 6 }, reward: { gold: 18 }, result: 'The cave shouts back — hard enough to knock dust and old coins from the ceiling. You gather what you can before the next echo arrives.' },
    ],
  },
};
...
const RARE_CARD_TEMPLATES = {
  ...
  elder_spirit: {
    name: 'Elder Spirit', type: CARD_TYPES.SKILL, element: null,
    cost: 1, damage: 0, block: 8,
    effects: [{ type: 'heal', value: 6 }, { type: 'draw', value: 1 }],
...
function createCampaignState() {
  ...
  return {
    ...
    npcRewardsCollected: new Set(), // track one-time NPC rewards
  };
}
